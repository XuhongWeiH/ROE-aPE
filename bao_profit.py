...
import pandas as pd
import time
def computeProfit(code, year, quarter):
 # 查询杜邦指数
    lg = bs.login()
    dupont_list = []
    rs_dupont = bs.query_profit_data(code, year, quarter)
...
def compute_total_Profit(yearSE):
    # 登陆系统
    lg = bs.login()
    # 显示登陆返回信息
    ...
    while (rs.error_code == '0') & rs.next():
        ...
        for year in range(yearSE[0], yearSE[1]):
            ...
            for season in range(1,5):
                df = computeProfit(code, year, season)
                if df.empty:
                    continue
                else:
                    df['season']=season
    ...
    # 原始数据存储
    result_profit.to_csv("./data/Profit_"+str(yearSE)+".csv",encoding="utf-8", index=False)
    # 筛选有用数据
...
if __name__ == '__main__':
     compute_total_Profit(yearSE = [2015, 2020])