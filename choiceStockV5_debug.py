...
cangwei_feature_step = 10.1#
cangwei_real_step = 0.1
...
def select_code(roe_dic, pe_dic):
    ...
    for k in list(roe_dic.keys()) + list(pe_dic.keys()):
        ...
        try:
            if roe_dic[k] > -100 and pe_dic[k] > -1:
                ...
                bodong_code_dic[k] = (roe_dic[k] + para3_bodong*pe_dic[k],
                                            roe_dic[k],
                                            pe_dic[k],
                                            )
            else:
    ...
    return result, select_code_dic, bodong_code_dic
...
def buyAnalyse(sltDate, result, select_code_dic, bodong_code_dic):
    ...
    for r in result_small:
        ...
        #     continue
            ...
            chicang[r[0]] = {
                            ...
                            '指标初始': r[1][0],
                            '今日指标': r[1][0],
                            '初始波动': bodong_code_dic[r[0]][0],
                            '波动指标': bodong_code_dic[r[0]][0],
                            'ROE': r[1][1],
                            'PE': r[1][2],
                            '当前成本': price,
                            '当前价格': price,
            ...
            logging.debug(','.join(['明日操作', '建仓',
                                   ...
                                   '指标初始', str(r[1][0]),
                                   '今日指标', str(r[1][0]),
                                   '初始波动', str(bodong_code_dic[r[0]][0]),
                                   '波动指标', str(bodong_code_dic[r[0]][0]),
                                   '当前成本', str(price),
                                   '当前价格', str(price),
                                   '持仓数量', str(stock_buy),
...
def holdAnalyse(sltDate, select_code_dic, bodong_code_dic):
    ...
    for item in chicang.keys():
        try:
            new_feature = bodong_code_dic[item][0]
        except KeyError:
            print(item, '没有 指标信息')
            new_feature = -998

        chicang_tmp = chicang[item]
        last_feature = chicang_tmp['波动指标']
        origin_feature = chicang_tmp['初始波动']
        ...
        chicang[item] = {'操作日期': daysAgo(sltDate,-1),
                        ...
                        '今日指标': new_feature,
                        '初始波动': chicang_tmp['初始波动'],
                        '波动指标': bodong_code_dic[item][0],
...
if __name__ == '__main__':
    ...
    while sltDate < '2019-04-29':#今日日期，预测明日
        ...
        result, select_code_dic, bodong_code_dic = select_code(roe_dic, pe_dic)
        buyAnalyse(sltDate, result, select_code_dic, bodong_code_dic)
        holdAnalyse(sltDate, select_code_dic, bodong_code_dic)
